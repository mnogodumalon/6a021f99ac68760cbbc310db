...
interface TestDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Test['fields']) => Promise<void>;
  defaultValues?: Test['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function TestDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: TestDialogProps) {
  const [fields, setFields] = useState<Partial<Test['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'test');
      await onSubmit(clean as Test['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Test['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Edit Test' : 'New Test';
