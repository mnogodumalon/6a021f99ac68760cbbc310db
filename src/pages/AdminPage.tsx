...
import { TestViewDialog } from '@/components/dialogs/TestViewDialog';
...
const ENTITY_TABS = [
  { key: 'test', label: 'Test', pascal: 'Test' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('test');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'test': return (data as any).test as Test[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'test': return {
        create: (fields: any) => LivingAppsService.createTestEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTestEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTestEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'test' || dialogState?.entity === 'test') && (
        <TestDialog
          open={createEntity === 'test' || dialogState?.entity === 'test'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'test' ? handleUpdate : (fields: any) => handleCreate('test', fields)}
          defaultValues={dialogState?.entity === 'test' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Test']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Test']}
        />
      )}
      {viewState?.entity === 'test' && (
        <TestViewDialog
          open={viewState?.entity === 'test'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'test', record: r }); }}
        />
      )}
