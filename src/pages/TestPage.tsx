...
export default function TestPage() {
  ...
  const [viewingRecord, setViewingRecord] = useState<Test | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getTest());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Test['fields']) {
    await LivingAppsService.createTestEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Test['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateTestEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Test"
      subtitle={`${records.length} Test in the system`}
      action={
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                  {search ? 'No results found.' : 'No Test yet. Add one!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
